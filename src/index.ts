...
server.tool(
  "email_send",
  "Send an email via SMTP. Supports text and HTML body, CC, BCC, reply-to, and file attachments (via URL or base64 only).",
  {
...
server.tool(
  "email_folders",
  "List all email folders/mailboxes (e.g. INBOX, Sent, Trash, Drafts).",
  {},
  async () => {
    try {
      return await withImap(async (client) => {
        ...
        const lines = folders.map((f) => {
          const flags = [];
          if (f.specialUse === "\\Sent") flags.push("Gesendet");
          if (f.specialUse === "\\Trash") flags.push("Papierkorb");
          if (f.specialUse === "\\Drafts") flags.push("Entwürfe");
          if (f.specialUse === "\\Junk") flags.push("Spam");
          if (f.specialUse === "\\Archive") flags.push("Archiv");
...
server.tool(
  "email_list",
  "List recent emails in a folder. Returns newest first with UID, date, sender, subject, and read status.",
  {
  ...
  async (params) => {
    try {
      return await withImap(async (client) => {
        ...
        const messages: Array<{ uid: number; date: string; from: string; subject: string; flags: string }> = [];

        for await (const msg of client.fetch(`${start}:*`, { envelope: true, flags: true, uid: true })) {
          const env = msg.envelope;
          const flagList = [];
          if (msg.flags && !msg.flags.has("\\Seen")) flagList.push("ungelesen");
          if (msg.flags?.has("\\Answered")) flagList.push("beantwortet");
          if (msg.flags?.has("\\Flagged")) flagList.push("★");
        ...
        const lines = messages.map((m) =>
          `  • [UID ${m.uid}] ${m.date} | ${m.from} | ${m.subject}${m.flags}`
        );
...
server.tool(
  "email_read",
  "Read a specific email by UID. Returns headers, text body, and attachment list. Use email_list first to get UIDs.",
  {
...
server.tool(
  "email_search",
  "Search emails by sender, subject, body text, date range, or read status. Returns matching UIDs with summary.",
  {
  ...
  async (params) => {
    try {
      return await withImap(async (client) => {
        ...
        const messages: Array<{ uid: number; date: string; from: string; subject: string }> = [];
        ...
        const lines = messages.map((m) =>
          `  • [UID ${m.uid}] ${m.date} | ${m.from} | ${m.subject}`
        );
...
server.tool(
  "email_delete",
  "Delete an email. Moves to Trash by default, or permanently deletes if permanent=true.",
  {
...
server.tool(
  "email_reply",
  "Reply to an email. Reads the original via IMAP, then sends the reply via SMTP with proper headers (In-Reply-To, References, Re: subject).",
  {
  ...
  async (params) => {
    try {
      ...
      const original = await withImap(async (client) => {
        ...
        if (!source) throw new Error(`Email mit UID ${params.uid} nicht gefunden.`);
      ...
      if (!replyTo) return errorResponse("Absenderadresse des Originals nicht gefunden.");
